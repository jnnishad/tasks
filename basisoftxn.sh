...
a=`grep -A 2  $i $1|grep AccountNo|head -1|tr -d '<>/'|grep -o "[[:digit:]]\+"|sed "s/\(......\)\(......\)/\1\XXXXXX/g"`
...
brrn=` grep --color=auto -A 1  $i $1 |grep OrderNo | tr -d '<>/'|awk '{print$NF}'|grep -o "[[:digit:]]\+"|tail -1`

#tid=`grep -B 12 "</x:NetworkRequest> for - $i" $i |grep Trans|head -1|awk -F '<' '{print$2}'|awk -F '>' '{print$2}'`
tid=`grep -B 12 "</x:NetworkRequest> for - $i" $1 |grep '<TransData>........</TransData>'|tail -1|awk -F '<' '{print$2}'|awk -F '>' '{print$2}'`
#if [ -z "$tid" ]
#then 
#tid=`grep -A 8 $i $i|grep '<TransData>1091....</TransData>'|tail -1|awk -F '<' '{print$2}'|awk -F '>' '{print$2}'`
echo -e "update transaction_log set cardnumber='"$a"',response_code='0000',txn_status='OK',rrn='"$brrn"',stage='Transaction Success',user_data_27='00' where transaction_id='"$i"';   -- $i "
#fi
done
